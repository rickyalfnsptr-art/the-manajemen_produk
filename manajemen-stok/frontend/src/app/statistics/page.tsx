  ...
  return (
    ...
    >
      <div className="h-full flex flex-col justify-between gap-3 overflow-hidden">
        {/* 1. Filter Bar */}
        <div className="bg-white p-3 sm:p-3.5 rounded-lg border border-slate-200 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-3 flex-shrink-0">
          <div>
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            ...
            <button
              ...
              disabled={loading}
              title="Perbarui Data"
              className="p-2.5 bg-slate-50 hover:bg-slate-100 border border-slate-200 rounded-lg text-slate-600 hover:text-slate-900 transition-colors shadow-2xs flex-shrink-0"
            >
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 flex-shrink-0">
          {/* Total Part */}
          <div className="p-3.5 rounded-lg bg-white border border-slate-200 shadow-sm flex flex-col justify-between hover:border-blue-300 transition-all">
          ...
          {/* Normal Stock (Aman) */}
          <div className="p-3.5 rounded-lg bg-white border border-slate-200 shadow-sm flex flex-col justify-between hover:border-emerald-300 transition-all">
            <div className="flex items-center justify-between gap-2 mb-1.5">
        ...
        {/* 3. Main Graphical Views: Bar Chart & Donut Chart (Zero-Scroll Fit) */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-3.5 flex-1 min-h-0">
          {/* IN vs OUT 7-Day Trend Chart (8 Cols) */}
          <div className="lg:col-span-8 bg-white rounded-lg border border-slate-200 p-4 shadow-sm flex flex-col justify-between">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-2.5 mb-1 border-b border-slate-100 flex-shrink-0">
              <div>
              ...
              <div className="flex flex-wrap items-center gap-1.5 text-xs">
                <span className="font-bold text-white bg-blue-600 px-2.5 py-0.5 rounded-md text-xs shadow-2xs">
                  IN: {totalInSum.toLocaleString('id-ID')}
                </span>
                <span className="font-bold text-white bg-emerald-600 px-2.5 py-0.5 rounded-md text-xs shadow-2xs">
                  OUT: {totalOutSum.toLocaleString('id-ID')}
                </span>
                <span className="font-bold text-white bg-slate-900 px-2.5 py-0.5 rounded-md text-xs shadow-2xs">
          ...
          <div className="lg:col-span-4 bg-white rounded-lg border border-slate-200 p-4 shadow-sm flex flex-col justify-between">
            <div className="flex items-center gap-2 pb-2.5 mb-1 border-b border-slate-100 flex-shrink-0">
              <div>
            ...
            <div className="h-32 sm:h-36 lg:h-[calc(100vh-440px)] min-h-[130px] max-h-[180px] w-full relative">
              {totalStatus > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie
            ...
            {/* Breakdown List with Mini Progress Bars */}
            <div className="space-y-2 pt-2 border-t border-slate-100 text-xs flex-shrink-0">
              ...
              <div className="space-y-1">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-slate-700 font-semibold">Aman (Normal)</span>
                  </div>
                  <span className="font-bold text-slate-900 font-mono">
                    {stats?.normalStockCount || 0} <span className="text-slate-400 font-normal">({normalPercentage}%)</span>
                  </span>
                </div>
                <div className="w-full bg-slate-100 rounded-full h-1.5 overflow-hidden">
                  <div className="bg-emerald-500 h-full rounded-full transition-all duration-500" style={{ width: `${normalPercentage}%` }} />
                </div>
              </div>
              ...
              <div className="space-y-1">
                ...
                <div className="w-full bg-slate-100 rounded-full h-1.5 overflow-hidden">
                  <div className="bg-red-500 h-full rounded-full transition-all duration-500" style={{ width: `${criticalPercentage}%` }} />
                </div>
              </div>
              ...
              <div className="space-y-1">
                ...
                <div className="w-full bg-slate-100 rounded-full h-1.5 overflow-hidden">
                  <div className="bg-amber-500 h-full rounded-full transition-all duration-500" style={{ width: `${overPercentage}%` }} />
                </div>
              </div>
            </div>
          </div>
